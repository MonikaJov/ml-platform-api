...
dataset('classification problem detail data', [
    function () {
        $file = UploadedFile::fake()->createWithContent('data.csv',
            'id,name,email,gender'.PHP_EOL
            .'1,Ana,ana@example,f'.PHP_EOL
            .'2,Bob,bob@example,m'.PHP_EOL
            .'3,Cara,cara@example,f'.PHP_EOL
        );
...
dataset('regression problem detail data', [
    function () {
        $file = UploadedFile::fake()->createWithContent('data.csv',
            'id,name,experience,salary'.PHP_EOL
            .'1,Ana,3,2000'.PHP_EOL
            .'2,Bop,1,1000'.PHP_EOL
        );
        $file->storeAs("{$this->user->id}", 'data.csv', 'datasets');
...
it('cannot store if target column is not suitable for classification', function () {
    $file = UploadedFile::fake()->createWithContent('data.csv', 'id,name,email,gender'.PHP_EOL.'1,Ana,ana@example.com,f'.PHP_EOL);
    $file->storeAs("{$this->user->id}", 'data.csv', 'datasets');
    ...
    expect($response->status())->toBe(422)
        ->and($response->json('errors')['target_column'])->toContain('Column \'gender\' is not suitable for classification; it needs at least two distinct labels.');
});

it('cannot store if target column is not suitable for binary classification', function () {
    $file = UploadedFile::fake()->createWithContent('data.csv', 'id,name,email,gender'.PHP_EOL.'1,Ana,ana@example.com,f'.PHP_EOL);
    $file->storeAs("{$this->user->id}", 'data.csv', 'datasets');
    ...
    $response = $this->postJson(route($this->routeName, [
        ...
        'type' => ProblemDetailTypeEnum::BINARY_CLASSIFICATION->value,
        'target_column' => 'gender',
        'dataset_id' => $dataset->id,
    ]);

    expect($response->status())->toBe(422)
        ->and($response->json('errors')['target_column'])->toContain('Column \'gender\' is not suitable for binary classification; it must have exactly two distinct labels.');
});
...
it('cannot store if dataset is unreadable', function () {
    $response = $this->postJson(route($this->routeName, [
        'dataset' => Dataset::factory()->create()->id,
    ]), [
        'type' => ProblemDetailTypeEnum::CLASSIFICATION->value,
